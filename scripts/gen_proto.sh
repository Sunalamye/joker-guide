...
if "${PYTHON_BIN}" -c "import grpc_tools.protoc" >/dev/null 2>&1; then
...
elif command -v protoc >/dev/null 2>&1 && command -v protoc-gen-grpc_python >/dev/null 2>&1; then
  ...
  protoc -I "${ROOT_DIR}/proto" \
    --python_out "${ROOT_DIR}/python-env/src/joker_env/proto" \
    "${ROOT_DIR}/proto/joker_guide.proto"
  echo "Warning: grpc Python plugin not found. Generated pb2 only; pb2_grpc not updated." >&2
else
  echo "protoc not found. Install protoc or grpcio-tools." >&2
  exit 1
fi
...
grpc_file = pathlib.Path(r"${GRPC_FILE}")
